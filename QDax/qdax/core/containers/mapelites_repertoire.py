...
class MapElitesRepertoire(flax.struct.PyTreeNode):
    ...
    def add(
        ...
        # get the max value for each segment (each cell)
    ...
    def init_default(
        ...
        # default genotypes is all jnp.nan
        default_genotypes = jax.tree_util.tree_map(
            lambda x: jnp.full(shape=(num_centroids,) + x.shape, fill_value=jnp.nan, dtype=x.dtype),
            genotype,
        )

        # default descriptor is all jnp.nan
        default_descriptors = jnp.full_like(centroids, jnp.nan)
